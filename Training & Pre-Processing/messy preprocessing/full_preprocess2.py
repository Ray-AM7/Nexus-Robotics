...
def main():
    ...
    # ----- Exclusion & Remapping -----
    # --- Step 1: Combine gestures ---
    # For example, combine gesture 15 with gesture 0.
    CombinedFeatSet[0].extend(CombinedFeatSet[15])
    CombinedLabelSet[0].extend(CombinedLabelSet[15])
    print(len(CombinedFeatSet[0]))
    print(len(CombinedLabelSet[0]))
    print(len(CombinedFeatSet[2]))
    print(len(CombinedLabelSet[2]))

    # Remove dupe gesture 15 entirely.
    del CombinedFeatSet[15]
    del CombinedLabelSet[15]
    print(len(CombinedFeatSet))
    print(len(CombinedLabelSet))
    
    # --- Step 2: Exclude unwanted gestures ---
    # For example, exclude gestures 1, 3, 4, and 6 (based on the new ordering after combination).
    excludeGestures = [1, 3, 4, 6]
    keep_indices = [i for i in range(len(CombinedFeatSet)) if i not in excludeGestures]
    ...
    # --- Step 3: Remap labels to sequential numbers ---
    # For each remaining gesture cell, set its labels to the new sequential number.
    new_total = len(CombinedFeatSet)
    for new_label, old_idx in enumerate(keep_indices):
        # All windows in the cell corresponding to old_idx get new_label.
        # Find the cell in CombinedLabelSet that is now at position corresponding to new_label.
        # Since we've filtered the lists, they are in the same order as keep_indices.
        cell_labels = CombinedLabelSet[new_label]
        CombinedLabelSet[new_label] = [new_label] * len(cell_labels)
    
    # Update NGESTURE to new number.
    NGESTURE = new_total
    ...
    train_ratio = 0.7  # e.g., 70% for training, 30% for testing
    ...
    for g in range(NGESTURE):
        ...
        print(np.size(gesture_windows))
        print(np.size(gesture_labels))
        ...
        # Determine the split index based on the percentage.
        num_windows = gesture_windows.shape[0]
        train_count = int(train_ratio * num_windows)
        
        # Split indices (adjust numbers if needed)
        # Taking the first 20038 windows for training and the next 10019 for testing.
        train_windows = gesture_windows[:train_count]
        train_labels = gesture_labels[:train_count]
        test_windows = gesture_windows[train_count:]
        test_labels = gesture_labels[train_count:]
